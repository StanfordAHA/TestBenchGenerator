...
parser.add_argument('--pnr-io-collateral', metavar='<collateral_file>.io.json', help='Collateral file generated by SMT-PNR')
parser.add_argument('--bitstream', metavar='<BITSTREAM_FILE>', help='Bitstream file containing the CGRA configuration')
...
for module in io_collateral:
    ...
    if mode == 'in':
        input_body += f"""
            ...
            CGRA_tester->{module} = {module}_in;
        """
    else:
        output_body += f"""
            {module}_out = CGRA_tester->{module};
...
void step(VCGRA_tester *CGRA_tester) {{
    CGRA_tester->clk_in ^= 1;
    CGRA_tester->eval();
}}

int main(int argc, char **argv) {{
    Verilated::commandArgs(argc, argv);
    VCGRA_tester* CGRA_tester = new VCGRA_tester;

    {file_setup}

    CGRA_tester->clk_in = 0;
    CGRA_tester->config_addr_in = 0;
    CGRA_tester->config_data_in = 0;
    std::cout << "Initializing the CGRA by holding reset high for " << NUM_RESET_CYCLES << "cycles" << std::endl;
    CGRA_tester->reset_in = 1;
    ...
    for (int i = 0; i < {len(config_data_arr)}; i++) {{
        CGRA_tester->config_data_in = config_data_arr[i];
        CGRA_tester->config_addr_in = config_addr_arr[i];
        next(CGRA_tester);
    }}
